const sourceActivation = latestCompletedActivation("extractClaim");
const claim = materializeAgentHandoff(sourceActivation, {
	status: "claim_materialized",
	producerNode: "materializeClaim",
	sourceNode: "extractClaim",
	artifact: "workflow-output/research-claim.json",
});

await Bun.write(claim.artifact, `${JSON.stringify(claim.value, null, 2)}\n`);

return {
	summary: `materialized research claim for reproduction at ${claim.artifact}`,
	statePatch: [{ op: "set", path: "/claim", value: claim.value }],
	data: {
		artifact: claim.artifact,
		source_activation_id: sourceActivation.id,
	},
	artifacts: [`local://${claim.artifact}`],
};
