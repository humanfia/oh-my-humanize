...
The main agent is always `Main`. Subagents inherit their task ID (e.g., `AuthLoader`, `AuthLoader-2` on repeat). If you don't know who is currently running, use `op: "list"` to view all peers alongside their status (`running` | `idle` | `parked` | `history-only`), unread message count, parent, and recent activity. Address peers by their exact ID from the roster; NEVER invent names. `history-only` peers are finished transcript refs; read `history://<id>` instead.
